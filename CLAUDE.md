...
> Read `.private/PLAN.md` first (owner-private, gitignored; ask the owner if you do not have it).
> It says what we are building, in what order, and what is cut.
> `SECURITY.md` is the threat model and the non-goals. `CONTRIBUTING.md` is the workflow.
> `.private/AUDIT-2026-09-20.md` says why the base code cannot be trusted with an agent yet.
...
`LICENSE.md` forever. Every upstream name, host, key and affiliate ID is replaced in Phase 2.
...
9. **Scope is `.private/PLAN.md`.** Cut items are not pulled forward because they are more fun than
   finishing the current phase.
10. **Visual: dark only, one mark, one accent.** `docs/BRAND.md` is the authority. The mark is
    `brand/mark.svg`, exported by script, never hand-drawn, never generated by an image model, and
    there is no light-mode variant to build.
11. **Use it before you rebuild it.** The forked wallet works. Changes are one of three kinds:
    make it ours (name, brand, remove the parent's endpoints), make it safe for an agent (an
    audit finding, with a test), or make it smaller (disable first; delete only when the compiler
    proves nothing references it). No deletion sprees, no rewriting a module because we would
    have written it differently, no reconstruction of what already runs. `.private/PLAN.md` §0.
